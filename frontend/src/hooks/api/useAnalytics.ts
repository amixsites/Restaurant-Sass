...
export const useAnalytics = (range: string = 'Weekly') => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      logger.start('ANALYTICS', 'FETCH', `Direct Supabase analytics for restaurant ${restaurantId} (Range: ${range})`);
      ...
      try {
        ...
        logger.success('ANALYTICS', 'FETCH', `Direct Supabase analytics fetched successfully`);
        return {
          revenue: {
        ...
        logger.error('ANALYTICS', 'FETCH', err);
        throw err;
      }
    },
