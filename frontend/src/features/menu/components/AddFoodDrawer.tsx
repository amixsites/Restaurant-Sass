...
export const AddFoodDrawer = ({ isOpen, onOpenChange, categories, itemToEdit, onSave }: AddFoodDrawerProps) => {
  ...
  const handleSave = async () => {
    logger.start('MENU', 'VALIDATE_FOOD_FORM', 'Validating food form', formData);
    try {
      setIsSubmitting(true);
      logger.debug('MENU', 'FOOD_FORM_SUBMIT_START', 'Food form submit started', {
      ...
      if (selectedFile) {
        logger.info('MENU', 'FOOD_IMAGE_UPLOAD_START', 'Uploading food image');
        const uploadedUrl = await uploadImage(selectedFile);
        if (uploadedUrl) {
          finalImageUrl = uploadedUrl;
          logger.success('MENU', 'FOOD_IMAGE_UPLOAD_SUCCESS', 'Food image uploaded', { uploadedUrl });
        } else {
          logger.warn('MENU', 'FOOD_IMAGE_UPLOAD_EMPTY', 'Image upload returned empty URL');
        }
      }
      
      logger.debug('MENU', 'FOOD_FORM_SAVE_PAYLOAD', 'Calling onSave with payload', {
        ...formData,
      ...
      logger.success('MENU', 'FOOD_FORM_SUBMIT_SUCCESS', 'Food form submit completed');
      
      // onOpenChange is called from MenuManagement on success
    } catch (error: any) {
      logger.error('MENU', 'SAVE_FOOD_DRAWER_ERROR', error);
    } finally {
