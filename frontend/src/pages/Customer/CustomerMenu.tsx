...
export const CustomerMenu = () => {
      ...
      try {
        if (qrToken) {
          ...
          const { data: restData, error: restError } = await supabase
            .from('restaurants')
            .select('id, name, customer_info_mode')
          ...
          setCustomerInfoMode(restData.customer_info_mode || 'name_phone');
          ...
          const { data: restData } = await supabase
            .from('restaurants')
            .select('name, customer_info_mode')
            .eq('id', routeRestaurantId)
            .single();
          if (restData?.name) setRestaurantName(restData.name);
          if (restData?.customer_info_mode) setCustomerInfoMode(restData.customer_info_mode);
          ...
          const { data: sess, error: sessErr } = await supabase
            ...
            .select(`
              restaurant_id,
              table_id,
              restaurants ( name, customer_info_mode ),
              tables ( table_number )
            `)
            .eq('session_id', activeSessionId)
            .single();
          ...
          const cInfoMode = (sess.restaurants as any)?.customer_info_mode || 'name_phone';

          setRestaurantId(rId);
          setTableId(tId);
          setRestaurantName(rName);
          setTableNumber(tNum);
          setCustomerInfoMode(cInfoMode);
  ...
  if (customerInfoMode === 'name_phone') {
    showForm = !customerName || !customerPhone;
    requireName = true;
    requirePhone = true;
  } else if (customerInfoMode === 'name_only') {
    showForm = !customerName;
    requireName = true;
  } else if (customerInfoMode === 'phone_only') {
    showForm = !customerPhone;
    requirePhone = true;
  } else if (customerInfoMode === 'none') {
    showForm = false;
  }

  const validRegistration = 
    (requireName ? name.trim().length > 1 : true) && 
    (requirePhone ? mobile.length >= 10 : true);

  const submitRegistration = () => {
    if (!validRegistration) return;
    setCustomerDetails(requireName ? name : '', requirePhone ? mobile : '');
  };
    ...
    return (
      <MobileShell>
        <div className="flex-1 overflow-y-auto no-scrollbar px-6 pt-10 pb-32">
          ...
          >
            {requireName && <FloatingInput label="Your Name" value={name} onChange={setName} icon={User} />}
            {requirePhone && <FloatingInput label="Mobile Number" value={mobile} onChange={(v) => setMobile(v.replace(/\D/g, "").slice(0, 10))} icon={Phone} type="tel" inputMode="tel" />}
